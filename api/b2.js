...
async function authUser(req) {
  ...
  if (!username) {
    throw new Error('Invalid LMS account');
  }

  const r = await fetch(
    `${databaseUrl}/users/${encodeURIComponent(username)}.json?auth=${encodeURIComponent(token)}`
  );

  if (!r.ok) {
    throw new Error('Could not verify LMS profile');
  }

  const user = await r.json();

  if (!user) {
    throw new Error('LMS profile not found');
  }
...
export default async function handler(req, res) {
  try {
    ...
    if (action === 'upload') {
      if (!['admin', 'content'].includes(user.role)) {
        return json(res, 403, {
          error: 'Admin/Content access required'
        });
      }
      ...
      if (
        !['corporate', 'consumer', 'technical', 'non-telecom'].includes(dept) ||
        !['rateplan', 'service', 'process', 'tnps'].includes(topic) ||
        !id
      ) {
        return json(res, 400, {
      ...
      if (
        ...
        return json(res, 400, {
          error: 'Only video and PDF uploads are supported'
        });
      }

      const storageKey =
        `content/${dept}/${topic}/${id}/${Date.now()}-${name}`;
